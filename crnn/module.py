...
class ConvRNNCellBase(torch.nn.Module):         
    ...
    def forward(self, input, hx=None):
        ...
        if self.mode == 'PeepholeLSTM':
            return cell(
                input, hx,
                self.weight_ih, self.weight_hh, self.weight_pi,
                self.weight_pf, self.weight_po,
...
class LSTMdistCell(torch.nn.Module):
    """ 
    ...
    def __init__(self, mode: str,
        ...
        super(LSTMdistCell, self).__init__()
        #assert mode == 'LSTMdis'
        self.mode = mode
        self.groups = groups
        self.in_channels = in_channels
        ...
        self.bias = bias
        self.convndim = convndim
        self.linear_func = ConvNdWithSamePadding(convndim=convndim, stride=stride, dilation=dilation, groups=groups)
        if convndim == 1:
            ntuple = _single
        elif convndim == 2:
            ntuple = _pair
        ...
        self.weight_th1 = Parameter(torch.Tensor(Tgate_size, in_channels // groups))
        self.weight_mh1 = Parameter(torch.Tensor(Tgate_size, out_channels // groups))
        self.weight_th2 = Parameter(torch.Tensor(Tgate_size, in_channels // groups))
        self.weight_mh2 = Parameter(torch.Tensor(Tgate_size, out_channels // groups))
        self.weight_to = Parameter(torch.Tensor(Tgate_size, out_channels // groups))
        ...
        if bias:
            self.bias_ih = Parameter(torch.Tensor(gate_size))
            self.bias_hh = Parameter(torch.Tensor(gate_size))
            self.bias_th1 = Parameter(torch.Tensor(Tgate_size))
            self.bias_th2 = Parameter(torch.Tensor(Tgate_size))

        else:
            self.register_parameter('bias_ih', None)
            self.register_parameter('bias_hh', None)
            self.register_parameter('bias_th1', None)
            self.register_parameter('bias_th2', None)
        ...
        nn.init.normal_(self.a.data, mean=0.3, std=0.001)
        nn.init.normal_(self.b.data, mean=0.3, std=0.001)
        nn.init.normal_(self.c.data, mean=0.1, std=0.001)
        nn.init.normal_(self.d.data, mean=1, std=0.01)
        nn.init.normal_(self.e.data, mean=1, std=0.01)
        nn.init.normal_(self.f.data, mean=0.1, std=0.01)
    ...
    def forward(self, input, time_dis,  hidden = None):
        self.check_forward_input(input)
        if hidden is None:
            batch_size = input.size(0)
            insize = input.shape[2:]
        ...
        cell = _conv_cell_helper(
            ...
            groups=self.groups)
        return cell(
            input, time_dis, hidden, self.a, self.b, self.c, self.d, self.e, self.f,
            self.weight_ih, self.weight_hh,self.weight_th1, self.weight_mh1, self.weight_th2,
            self.weight_mh2,self.weight_to, self.weight_pi,self.weight_pf, self.weight_po,
            self.bias_ih, self.bias_hh, self.bias_th1,  self.bias_th2,
        )
